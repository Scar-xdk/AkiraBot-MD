...
exports.execute = async (msg, sock, userId, { getDonos, setDonos }) => {
  ...
  if (!alvo) {
    await sock.sendMessage(msg.key.remoteJid, {
      text: "Marque ou responda alguém, ou envie o número para definir como Dono 2.",
    });
    return;
  }

  const numeroAlvo = alvo.replace(/[^0-9]/g, "");
  const novosDonos = { dono2: numeroAlvo, dono3 };
  setDonos(novosDonos);

  await sock.sendMessage(msg.key.remoteJid, {
    text: `Dono 2 atualizado com sucesso: @${numeroAlvo}`,
    mentions: [alvo],
  });
};
